...
class RGCNNet(nn.Module):
    ...
    def __init__(self, num_concepts, num_relations, num_basis, sent_dim, concept_dim,
                 d_k, d_v, num_gnn_layers, num_attention_heads, fc_dim, num_fc_layers, p_gnn, p_fc,
        ...
        self.pool_layer = MatrixVectorMultiHeadAttention(num_attention_heads, sent_dim, gnn_dim, d_k, d_v)
        self.fc = MLP(d_v * num_attention_heads + sent_dim, fc_dim, 1, num_fc_layers, p_fc, True)
...
class LMRGCN(nn.Module):
    def __init__(self, model_name, lstm_dic, **kwargs):
        super().__init__()
        self.model_name = model_name
        if model_name in ('lstm',):
            self.encoder = LSTMEncoder(**lstm_dic)
            sent_dim = lstm_dic['hidden_size']
        else:
            self.encoder = LMEncoder(model_name)
            sent_dim = self.encoder.sent_dim
        self.decoder = RGCNNet(**kwargs, sent_dim=sent_dim)
    ...
    def forward(self, *inputs, layer_id=-1):
        ...
        *lm_inputs, concept_ids, node_type_ids, adj_lengths, adj = inputs
        sent_vecs = self.encoder(*lm_inputs) if self.model_name in ('lstm',) else self.encoder(*lm_inputs, layer_id=layer_id)[0]
...
class LMRGCNDataLoader(object):
    ...
    def __init__(self, train_statement_path, train_adj_path,
                 ...
                 train_tokenized_path=None, dev_tokenized_path=None, test_tokenized_path=None, freq_cutoff=3, ):
        super().__init__()
        ...
        if model_type in ('gpt', 'bert', 'xlnet', 'roberta'):
            func, extra_args = load_input_tensors, [model_type, model_name, max_seq_length]
        elif model_type in ('lstm',):
            func, extra_args = load_info, []
        self.train_qids, self.train_labels, *self.train_data = func(train_statement_path, *extra_args)
        self.dev_qids, self.dev_labels, *self.dev_data = func(dev_statement_path, *extra_args)
        self.num_choice = None
        if test_statement_path is not None:
            self.test_qids, self.test_labels, *self.test_data = func(test_statement_path, *extra_args)
        if model_type in ('lstm',):
            self.num_choice = self.train_data[0]
            *self.train_data, vocab = load_tokenized_statements(tokenized_path=train_tokenized_path,
                                                                num_choice=self.num_choice,
                                                                max_seq_len=max_seq_length,
                                                                freq_cutoff=freq_cutoff, )
            self.vocab = vocab

            *self.dev_data, _ = load_tokenized_statements(tokenized_path=dev_tokenized_path,
                                                          num_choice=self.num_choice,
                                                          max_seq_len=max_seq_length,
                                                          freq_cutoff=freq_cutoff,
                                                          vocab=vocab, )
            if test_tokenized_path is not None:
                *self.test_data, _ = load_tokenized_statements(tokenized_path=test_tokenized_path,
                                                               num_choice=self.num_choice,
                                                               max_seq_len=max_seq_length,
                                                               freq_cutoff=freq_cutoff,
                                                               vocab=vocab, )
        if self.num_choice is None:
